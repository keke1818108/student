<template>
    <el-card class="student-root">
        <div class="top">
            <div class="search">
                <div class="s_cont">
                    <div class="tag">账号:</div>
                    <el-input v-model="query_sname" placeholder="请输入账号"></el-input>
                </div>
                <div class="s_cont">
        ...
        <div class="exam">
            <el-table :data="tableData" style="width: 100%">
                <el-table-column label="账号" width="150" prop="sname"></el-table-column>
        ...
        <el-dialog title="新增账号" class="addAccount" :visible.sync="addDialogVisible" width="30%"
            ...
            <el-form ref="resetForm" :rules="rules" :model="resetForm" label-width="100px">
                <el-form-item label="账号" prop="sName">
                    <el-input v-model="resetForm.sName" placeholder="请输入账号"></el-input>
                </el-form-item>
                <el-form-item label="密码" prop="sPwd">
                    <el-input v-model="resetForm.sPwd" placeholder="请输入密码"></el-input>
                </el-form-item>
                <el-form-item label="确认密码" prop="sPwdNew">
                    <el-input v-model="resetForm.sPwdNew" placeholder="请输入密码"></el-input>
                </el-form-item>
            </el-form>
        ...
        <el-dialog title="修改密码" :visible.sync="RestdialogVisible" width="30%" :before-close="pwdhandleClose">
            <el-form ref="resetForm" :rules="rules" :model="resetForm" label-width="100px">
                <el-form-item label="账号">
                    <el-input v-model="resetForm.sName" disabled></el-input>
                </el-form-item>
                <el-form-item label="密码" prop="sPwd">
                    <el-input v-model="resetForm.sPwd" placeholder="请输入密码"></el-input>
                </el-form-item>
        ...
        <el-dialog title="更新账号" :visible.sync="updateDialogVisible" width="40%">
            <el-form ref="updateForm" :rules="updateRules" :model="updateForm" label-width="100px">
                <el-form-item label="账号">
                    <el-input v-model="updateForm.sName" disabled></el-input>
                </el-form-item>
...
export default {
    ...
    data() {
        let validatePass2 = (rule, value, callback) => {
            ...
            } else if (value !== this.resetForm.sPwd) {
    ...
    methods: {
        ...
        handleEdit(index, row) {
            ...
            this.updateForm.sName = row.sname;
        ...
        updateSubmit() {
            ...
            this.$refs.updateForm.validate((valid) => {
                if (valid) {
                    this.$axios.post("/student/updateOne", this.row)
                        .then(res => {
        ...
        handleUpadtePwd(index, row) {
            this.row = row;
            this.resetForm.sName = row.sname;
            console.log(index, row, this.resetForm.sName);
            ...
            this.$refs.resetForm.validate((valid) => {
                if (valid) {
                    ...
                    this.$confirm('确认修改?', '提示', {
                        ...
                        this.row.spwd = this.resetForm.sPwd;
                        this.$axios.post("/student/updateOne", this.row)
                            .then(res => {
                                console.log(res.data);
                                if (res.data.status == '200') {
        ...
        handleDelete(index, row) {
            ...
            this.$confirm('是否该账号?', '提示', {
                ...
                this.$axios.post("/student/deleteOne", row)
                    .then(res => {
                        console.log(res.data);
                        if (res.data.status == '200') {
                            this.$message({
        ...
        addSubmit() {
            ...
            let newAccount = {
                sname: this.resetForm.sName,
                spwd: this.resetForm.sPwd,
                age: 0,
                enable: 1,
            ...
            this.$refs.resetForm.validate((valid) => {
                if (valid) {
                    this.$axios.post("/student/addOne", newAccount).then(res => {
                        console.log(res.data);
                        if (res.data.status == '200') {
                            this.$message({
                                message: "新增成功",
        ...
        statusChange(index, row) {
            ...
            this.$axios.post("/student/updateOne", row)
                .then(res => {
                    console.log(res.data);
                    if (res.data.status == '200') {
                        this.$message({
        ...
        */
        query(type) {
            ...
            if (this.query_nickName === '' && this.query_sname === '' && this.optionStatusValue === '') {
                this.$message({
                    message: "请输入检索条件",
                    type: "warning"
                })
        ...
        queryPageData(query) {
            // console.log(query);
            this.$axios.post("/student/queryStudentByPage", query)
                .then(res => {
                    // console.log(res.data);
