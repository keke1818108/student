    ...
    <el-card>
            ...
            <div class="search">
                ...
                <div class="s_cont">
                    ...
                    <el-select v-model="type" placeholder="请选择类型">
                        <el-option v-for="item in optionsType" :key="item.type" :label="item.label" :value="item.type">
                        </el-option>
                    </el-select>
                </div>
                <div class="s_cont">
        ...
        <el-dialog class="dialog" title="批量新增试题" :visible.sync="dialogVisible" width="40%">
            <div class="row">
                <span>课程名称：</span>
                <div>
                    <el-select v-model="optionsCourseValue" placeholder="请选择" @change="CourseOptionChange()" clearable>
                        <el-option v-for="item in optionsCourse" :key="item.cid" :label="item.courseName"
                            :value="item.cid">
                        </el-option>
                    </el-select>
...
export default {
    ...
    beforeMount() {
        ...
        //获得老师已教的课程
        if (!localStorage.getItem("optionsCourse")) {

            this.$axios.get("/teacher/getAllCourseByTid/" + this.userinfo.userData.tid)
                .then(res => {
                    console.log(res.data);
                    this.optionsCourse = res.data.data;
                    localStorage.setItem("optionsCourse", JSON.stringify(res.data.data))
                })
        } else {
            this.optionsCourse = JSON.parse(localStorage.getItem("optionsCourse"))
        }
    ...
    methods: {
        ...
        //新增试题
        submitAddQuestion() {
            console.log(this.optionsCourseValue, this.option_one_chapter, this.fileUpUrl);
        ...
        CourseOptionChange() {
            this.fileUpUrl = "http://localhost:8888/question/uploadQuestionByExcel?cid=" + this.optionsCourseValue +
        ...
        */
        query(query) {
            console.log("query()", this.query_course, this.query_chapter, this.type, this.query_cont,);
            // return
            //无条件查询
            if (query == 0) {
                this.getPageData({
                    courseName: this.query_course,
        ...
        //分页数据请求
        getPageData(query) {
            this.$axios.post("/question/getQuestionPageByTid?tid=" + this.userinfo.userData.tid, query).then(res => {
                // console.log(res.data);
